...
class MemoryBank(object):
    def __init__(self, n, dim, num_head, num_classes, temperature):
        self.n = n
        self.dim = dim 
        self.num_classes = num_classes
        self.num_head = num_head
        self.features = torch.FloatTensor(self.n, self.num_classes, self.dim)
        self.targets = torch.LongTensor(self.n, self.num_classes)
        self.ptr = 0
        self.device = 'cpu'
        self.K = 100
    ...
    def mine_nearest_neighbors(self, topk, calculate_accuracy=True):
        ...
        n, c, dim = features.shape[0], features.shape[1], features.shape[2]
        accuracy = 0
        indices_ = []
        for c_ in range(c):
            index = faiss.IndexFlatIP(dim)
            index = faiss.index_cpu_to_all_gpus(index)
            index.add(np.ascontiguousarray(np.squeeze(features[:,c_,:])))
            distances, indices = index.search(np.ascontiguousarray(np.squeeze(features[:,c_,:])), topk+1) # Sample itself is included
            indices_.append(indices)
            # evaluate 
            if calculate_accuracy:
                targets = self.targets.cpu().numpy()[:,c_]
                neighbor_targets = np.take(targets, indices[:,1:], axis=0) # Exclude sample itself for eval
                anchor_targets = np.repeat(targets.reshape(-1,1), topk, axis=1)
                accuracy += np.mean(neighbor_targets == anchor_targets)
        return indices_, accuracy/self.num_classes
    ...
    def update(self, features, targets):
        ...
        features = torch.cat([torch.unsqueeze(output,dim=1) for output in features],dim=1)            
        targets = targets[:,:self.num_head]
